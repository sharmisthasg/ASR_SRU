...
'''
#'C:\Users\marve\Desktop\SRU_plss bro\spoken-digit-recognition-master\spoken_digit'
#'spoken_digit/'
...
def build_dataset(sound_path='Digits/'):
    files = sorted(os.listdir(sound_path))
    x_train = []
    y_train = []
    x_test = []
    ...
    for f in files:
        feature = feature_extractor(sound_path=sound_path + f)
        # if(int(f[f.find("_")+1:f.find(".")]) == 5):
        #     continue
        x_train.append(feature)
        y_train.append(f[0])
    #print(y_train)
    #print("xxxx ", len(y_train))
    for i in range(0, len(x_train), len(x_train) // 10):
        data[y_train[i]] = x_train[i:i + len(x_train) // 10]
        #print(y_train[i])
    for k,v in data.items():
        x_test.append(v.pop(0))
        y_test.append(k)
        x_test.append(v.pop(0))
        y_test.append(k)
    return x_train, y_train, x_test, y_test, data
...
def feature_extractor(sound_path):
    ...
    mfcc_features = mfcc(audio, sampling_freq,nfft = 4096,numcep=39,nfilt=39,winfunc=np.hamming)
...
def train_model(data):
    ...
    for label in data.keys():
        model = hmm.GMMHMM(verbose=False,n_components=14,n_iter=1000)
...
for i in range(n):
    ...
    if int(y_pred) == int(y_test[i]):
        tot_test += 1
for i in range(m):
    y_pred = prediction(x_train[i], learned_hmm)
    if int(y_pred) == int(y_train[i]):
        tot_train += 1
print(tot_train/m)
print(tot_test/n)
'''Block 3 end'''
